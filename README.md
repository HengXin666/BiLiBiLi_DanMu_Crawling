...
- 📌 **历史全弹幕获取**: 内部爬取算法可在 **最少请求次数**下完成抓取, 并 **保证不丢失任何弹幕**.<sup>`[1]`</sup>
- 🔑 **多 Cookies 支持**: 可从 Cookies 池中随机选择一个作为请求凭证, **降低请求间隔**并提升抓取效率.<sup>`[2]`</sup>
- 🎥 **多类型视频支持**: 单视频、多 P 视频及番剧, 输入链接即可开始抓取
- 🗂️ **多任务管理**: 保存任务进度和爬取数据, 可按需启动或暂停任务
- 🛠️ **弹幕数据处理**:
    - 🔄 格式转换
    - 🧹 合并重复弹幕
    - 📊 统计分析(出现次数最多的内容、发言最多的用户 ID)
- 🐳 **Docker 部署**: 提供 Docker-Compose / DockerFile 一键私有部署方案(后端)

> **附注**:
>
> `[1]`: 原理详见 [#14](https://github.com/HengXin666/BiLiBiLi_DanMu_Crawling/issues/14) *(方法受 BiliPlus 全弹幕下载器启发)*
> 
> `[2]`: 使用多个 Cookies 轮流请求, 相当于多个账号交替访问。每个账号保持正常访问频率, 但整体请求速度更快, 从而有效降低被 B 站封禁的风险

## 二、功能展示 🎬

<div align="center">

[点击: 访问前端 (Github-Page)](https://hengxin666.github.io/BiLiBiLi_DanMu_Crawling/)
...
![](dev/img/Clip_2024-10-25_14-53-03.png)
