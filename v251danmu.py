...
class ProgressBar(object):#进度条
    DEFAULT = 'Progress: %(bar)s %(percent)3d%%'
    FULL = '请等待尝试! %(bar)s %(current)d/%(total)d (%(percent)3d%%)  剩余%(remaining)d'

    def __init__(self, total, width=45, fmt=DEFAULT, symbol='■',
                 output=sys.stderr):
        assert len(symbol) == 1
...
def jdt():#进度条设置
    if daily == 0:
        return
    progress = ProgressBar(daily, fmt=ProgressBar.FULL)
                        #设置格子数
    for x in range(progress.total):
        progress.current += 1
        progress()
        time.sleep(0.01)#设置时间
    progress.done()
...
def app_title():
    print(f'''
    ================================================================
    欢迎使用本程序! 本程序仅供学习与交流,由于不当操作产生的问题,责任自负!
    ================================================================
...
# \t[原作者: zjkwdy] https://github.com/zjkwdy/bilibili-history-danmu-spider
...
def get_cid():
    ...
    for video in req.json()['data']['pages']:
        print('======================================================================')
        print('视频 P'+str(video['page'])+' '+video['part']+' 的cid是 '+str(video['cid']))
    input('请按回车继续...')
...
def get_day_danmu(cid, date, SESSDATA):# 获取某天的历史弹幕!
    ...
    show_info(f'正在处理: {date} . | [成功]: {get_danmu_success_cs} , [尝试]: {get_danmu_fail_cs} | 还可请求: {max_get_dm} 次.')
...
def show_error(message):
    print(f'【错误】:[{time.strftime("%H:%M:%S")}]:{message}')
...
if __name__ == '__main__':
    ...
    try:#获取信息从配置文件中获取
        ...
        daily = int(config.items('spider')[3][1])*100
    except:
    ...
    if xz_ == 3:
        ...
        danmu_day_huanchun = f'缓存{cid}弹幕天数文件'
        show_info('「开始获取历史弹幕日期...时间较长耐心等待」だって')
        print(f'请输入保存获取弹幕天数弹幕的文件名:\n当前文件名为:[ {danmu_day_huanchun} ]\n为空 则使用当前文件名!')
        danmu_day_huanchun = input('请输入文件名:')
        if danmu_day_huanchun =='':
            danmu_day_huanchun = f'缓存{cid}弹幕天数文件'
        ...
        with open(f'{danmu_day_huanchun}.txt','w+',encoding='utf-8') as wj:
            pass
        for i in not_ok_day_danmu:
            with open(f'{danmu_day_huanchun}.txt','a+',encoding='utf-8') as wj:
                wj.write(str(i)+'\n')
        show_info(f'\t文件保存为【{danmu_day_huanchun}.txt】\n于路径:【 {os.getcwd()} 】中\n可以尝试等待几个小时,然后使用本程序的功能[4]接着爬取!\n([!]到时候请将本文件名输入至本程序)\n')
        print('\t-------------------------------')
        time.sleep(5)
