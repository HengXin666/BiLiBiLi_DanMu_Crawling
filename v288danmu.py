...
#     |20#调整: 颜色美化会使得进度条出现BUG.. 2.6.9
#     |21#调整: 修复了一些bug,增加体验. 2.7.0
#     |22#调整: 添加了一个测试功能的功能 2.8.0
#     |23#调整: 简单重构了一下代码 2.8.1
#
#     |[red]存在问题[/red]:
...
def search():#初始化工作区
    try:
        ...
        show_error('找不到文件 [yellow]HX配置文件.ini[/yellow] | 正在进行全盘搜索...')
        ...
        #root-路径 dirs-文件夹 files-文件
            if name in dirs or name in files:
                ...
                show_info(f'【成功找到文件!】初始化工作目录成功!\n当前工作目录: [yellow]{os.getcwd()}[/yellow]')
                return
        show_error('【错误】:找不到文件! | 缺少 【 HX配置文件.ini 】 文件 !')
        return
...
def cs(SESSDATA):#测试
    print('\n[red]温馨提示: 如果在测试过程中出现闪退 , 即说明无法正常使用 .[/red]\n')
    time.sleep(3)
    print(f'''
-----------当前数据-----------
    登录        SESSDATA = {SESSDATA}
    视频        cid = {cid}
    开始年份    start_year = {start_year}
    结束年份    end_year = {end_year}
    爬取延迟    daily = {daily}
    ''')
    time.sleep(3)
    if ip_dl == 1 :
        ...
        dl_sm = '关闭'
    show_info(f'当前代理状态: {dl_sm}\n')
    time.sleep(3)
    show_info('正在尝试获取 视频的弹幕日期缓存...')
    time.sleep(3)
    api_url = f'http://api.bilibili.com/x/v2/dm/history/index?type=1&oid=589796280&month=2022-05'
    req_headers = {
         'User-Agent': random_user_agent(),
         'Cookie': 'SESSDATA='+random_SESSDATA(SESSDATA)+';'
    }
    req_rq = requests.get(url=api_url, headers=req_headers).json()
    print(req_rq)
    ...
    if req_rq['code'] == 0:
        if req_rq['data'] != None:
            for date in req_rq['data']:
                nr.append(date)
                show_info(f'{date} 有弹幕！')
                time.sleep(0.05)
        else:
            show_info(f'2022-05：啥弹幕也木有嘞！(＞︿＜)')


    show_info('正在尝试获取 弹幕...')
    time.sleep(3)
    api_url = f'https://api.bilibili.com/x/v2/dm/web/history/seg.so?type=1&oid=589796280&date=2022-06-01'
    req_headers = {
        'User-Agent': random_user_agent(),
        'Cookie': 'SESSDATA='+random_SESSDATA(SESSDATA)+';',
        'Referer': 'https://www.bilibili.com/',
    ...
    data = requests.get(url=api_url, headers=req_headers)
    req = bilidm_pb2.DmSegMobileReply()
    req.ParseFromString(data.content)
    req_dm = req.elems
    print(req_dm)

    show_info('正在测试文件处理功能...')
    time.sleep(3)
    txt = '测试文件_HX_by测试功能_记得一键三连.txt'
    print(len(nr),type(nr),nr)

    while len(nr) > 0:
        with open(txt,'w+',encoding='utf-8') as wj:
            for i in nr:
                i = i.split('\n')[0]
                wj.write(f'{str(i)}\n')
                print('正在写入',i)
                time.sleep(0.05)
        print('-------------------------------------')
        with open(txt,'r',encoding='utf-8') as wj:
            nr = wj.readlines()
        print(nr)
        nr.pop(0)
        if len(nr) == 0:
...
if __name__ == '__main__':
    ...
    if xz_ == 3:
        ...
        if wj_cs_name =='.txt':
            wj_cs_name = f'缓存{cid}弹幕天数文件.txt'
